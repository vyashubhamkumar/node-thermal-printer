...
class ThermalPrinter {
  ...
  async execute(cb, userExecutionConfig) {
    try {
      let executionConfig = userExecutionConfig || {}
      this.Interface.execute(this.buffer, executionConfig)
      .then(jobId => {
        cb(null, jobId);
      })
      .catch(error => {
  ...
  async isPrinterConnected(exists) {
    this.Interface.isPrinterConnected()
    .then(() => exists(true))
    .catch(() => exists(false));
  }
